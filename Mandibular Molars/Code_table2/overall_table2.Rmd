...
table_data <- data.frame(
  Comparisons = c("Tarsius vs. Microcebus", "Tarsius vs. Mirza", "Tarsius vs. Saimiri",
                  "Microcebus vs. Mirza", "Microcebus vs. Saimiri", "Mirza vs. Saimiri",
                  "Tarsius vs. Tarsius"),
  Algorithm_1 = NA_character_,
  Algorithm_2 = NA_character_,
  Algorithm_4 = NA_character_
)
library(foreach)
library(doParallel)
library(iterators)
...
number_permutations=100
  ...
  chisq_statistic_seq=sum((sqrt(number_iterations)*apply(Xis, 2, mean))^2)
  df_seq=L
  p_value_seq[[perm]]=1-pchisq(sum((sqrt(number_iterations)*apply(Xis, 2, mean))^2), df=L)
  }
stopImplicitCluster()
stopCluster(cl)
...
table_data[7,2] = mean(p_value_final)
...
SECT_both_list = c(SECT_1_list,SECT_2_list)
 ###############################################################
  ...
  # We only care about the components having 95% cumulative variance.
  L_original=max(which(cumsum(result)/sum(result)<0.98))
  L=L_original
  Eigen_values=(T/grid_length)*result[1:L]
  Eigen_vectors=sqrt(grid_length/T)*eigen_results$vectors[,1:L]
...
for (i in 1:100) {
  ...
  table_data[7,3] = mean(p_value_final)
