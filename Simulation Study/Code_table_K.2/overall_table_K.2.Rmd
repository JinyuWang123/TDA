...
```{r}
table_data_algorithm1 <- data.frame(
  `Number of directions` = rep(c("Γ = 2", "Γ = 4", "Γ = 8"), each = 3),
  `Number of levels` = rep(c("Δ = 25", "Δ = 50", "Δ = 100"), 3),
  `n = 25` = NA_character_,
  `n = 50` = NA_character_,
  `n = 100` = NA_character_
)
...
number_directions_values=c(2,4,8)
grid_length_values=c(25,50,100)
number_iterations_values=c(25,50,100)

for (i_table in 1:length(number_directions_values)) {
  for (j_table in 1:length(grid_length_values)) {
    for (q_table in 1:length(number_iterations_values)) {
###############################################################
# Basic parameters
###############################################################
...
number_directions=number_directions_values[i_table]
# Number of levels t in [0, T]
grid_length=grid_length_values[j_table]
# Number of simulated shapes in each collection
number_iterations=number_iterations_values[q_table]
...
###############################################################
  ...
  p_value_a1[index_time] = p_value_seq
}
table_data_algorithm1[(i_table-1)*3+j_table,q_table+2]=paste(mean(p_value_a1), "(", sd(p_value_a1), ")")
}}}
...
```{r}
table_data_algorithm2 <- data.frame(
  `Number of directions` = rep(c("Γ = 2", "Γ = 4", "Γ = 8"), each = 3),
  `Number of levels` = rep(c("Δ = 25", "Δ = 50", "Δ = 100"), 3),
  `n = 25` = NA_character_,
  `n = 50` = NA_character_,
...
###############################################################
  ...
  #       lty=2)
  if(increasing_chisq_stat[1000]<chisq_statistic_original) {p_value_seq[studies] = 0}
  else {p_value_seq[studies]=1-(which(increasing_chisq_stat>chisq_statistic_original)[1])/1000}
}
#t2<-Sys.time()
#time[index_time]=t1-t2
  p_value_a2[index_time] = p_value_seq
}
table_data_algorithm2[(i_table-1)*3+j_table,q_table+2]=paste(mean(p_value_a1), "(", sd(p_value_a1), ")")
}}}
